...
import { FunnelProcessor, FunnelProcessorErrors } from "@/services/funnel-processor";
...
describe("useFunnelStore", () => {
	...
	it("should throw an error when adding an invalid file", async () => {
		(processorMock.readFunnelFromFile as jest.Mock).mockRejectedValueOnce(new Error("1"));
		const file = new File([""], "test.txt", { type: "text/plain" });
		await expect(useFunnelStore.getState().addFunnelFromFile(file)).rejects.toThrow();
	});
});